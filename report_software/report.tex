...
The goal of this project was to create a Python program that plays audio files from a specified folder in a random order, with the ability to navigate to the next track. The program utilizes the \texttt{pygame} library for audio playback and the \texttt{numpy} library for selecting songs randomly and \texttt{os} for importing the files.
...
\begin{itemize}
  ...
  \item Randomization: The program shuffles all the audi files randomly using \texttt{numpy} library.
  ...
  \item Next Track: Entering "next" in the terminal starts playing the next track.  
\end{itemize}
...
\begin{enumerate}
  ...
  \item Enter "next" in the terminal to go to the next track
...
\begin{itemize}
  ...
  \item \texttt{numpy}: Used for randomising the songs played.
\end{itemize}

Ensure that these libraries are installed in the Python environment before running the program.

\section{Conclusion}
The project successfully achieves its objective of creating a random audio player. The program allows users to enjoy their audio collection in a randomized order and provides convenient navigation options.
